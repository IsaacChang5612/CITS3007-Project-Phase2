...
# Group 40 checks:  data_offset + data_size > data_section_size, but both sides are u64, so if data_offset is close to UINT64_MAX the addition wraps around to 0 and the check always passes.
...
# F-02 - RLE size mismatch returns BUN_ERR_IO (3) instead of BUN_MALFORMED (1) validate_rle_data() does catch the mismatch correctly and returns BUN_ERR_CORRUPT, 
# but worst_error() has a bug where BUN_ERR_CORRUPT silently gets turned into BUN_ERR_IO instead.  The spec says this case should be BUN_MALFORMED (exit 1).
#
# Our RLE data is two pairs: [03 41] [05 42] which expands to 8 bytes (3 × 'A' + 5 × 'B').  We set uncompressed_size to 99, so the mismatch (8 vs 99) gets detected. 
# The error message is printed, but the wrong exit code comes out.
#
...
f02 += bytes([3, 0x41,        # pair 1: count=3, value='A'  3 bytes
              5, 0x42])       # pair 2: count=5, value='B'  5 bytes
...
# Group 40 checks three pairs of sections for overlap (asset table vs string table, asset table vs data section, string table vs data section) 
# but never compares any of them against the header which lives at [0, 60).
#
# Setting data_section_offset = 0 puts the data section right on top of the header.  All three checks pass fine because they don't involve the header, 
# so the parser carries on and reads the BUN magic bytes as the asset's data content — "BUN0...." — then exits 0.
#
# Expected: exit 1 (BUN_MALFORMED)   Actual: exit 0 (BUN_OK)
